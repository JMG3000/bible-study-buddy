...
| 2026-06-05 | Analytics and visual testing | Added Vercel Analytics, Meticulous App Router recorder wiring, deterministic date helper, and CircleCI validation/Meticulous jobs on `dev-test`. | Local lint, typecheck, build, and dependency audit passed. | Meticulous requires `METICULOUS_API_TOKEN` in CircleCI project secrets. Recorder capture loads in development and Vercel preview only. | Project maintainer |
| 2026-06-05 | GitHub security and promotion workflows | Added GitHub Actions for dev-test validation, advanced CodeQL, dependency review, secret-pattern checks, Scorecard, Dependabot, CODEOWNERS, and fast-forward promotion to production `main`. | Local diff check, lint, typecheck, build, and dependency audit passed. | GitHub branch protection and Vercel production branch settings must be confirmed in the web dashboards. | Project maintainer |
