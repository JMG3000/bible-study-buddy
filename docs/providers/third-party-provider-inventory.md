...
| CircleCI | Build verification for install, test, lint, typecheck, build, and dependency audit; triggerable from the Slack DevOps command endpoint. | CI runner and CircleCI API; branch filters cover `dev-test`, `main`, Dependabot branches, and `codex/bsbuddy-*` candidate branches. | `CIRCLECI_API_TOKEN` is server-only; project secrets stay in CircleCI contexts/env vars. | At the 2026-07-14 pre-push snapshot, YAML was repaired and parsed locally while queried remote state still reported the pre-repair `Unable to parse YAML`; current BSBUDDY-4 configuration admits isolated candidate branches |
| GitHub Actions | Supporting manual application validation and security review workflows; CircleCI remains the canonical application validator. | GitHub workflow service. | Repository/environment secrets remain in GitHub. | `Manual Application Validation` and `Security Review` source definitions are manual-dispatch only and have no branch-write authority; live workflow state remains provider-controlled and must be reverified after integration |
...
| Slack | Broadcast channel for integration connection status and DevOps metrics; signed validation and preview-deployment command input. Slack has no production deployment authority. | `#proj-bible-study-buddy` channel and `/api/devops/slack`. | `SLACK_SIGNING_SECRET` and webhook URLs are server-only. Do not post secrets, raw env values, private tokens, or service-role keys. | Endpoint/channel configured; settings and integration state unverified |
